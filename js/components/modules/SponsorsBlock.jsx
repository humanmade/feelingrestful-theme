			...
			<div className="Sponsors">
				<div className="inner">
						...
						return <section className={ 'sponsor-level ' + tier.name.toLowerCase() } key={tier.name}>
							...
							<ul>
								{tier.sponsors.map(sponsor => {
									return ( sponsor._embedded && sponsor._embedded['wp:featuredmedia'] &&
										<li key={sponsor.id}>
					...
					{ this.props.become_a_sponsor_button.link && this.props.become_a_sponsor_button.text ?
						<Link to={this.props.become_a_sponsor_button.link} className="Button">{this.props.become_a_sponsor_button.text}</Link>
					: '' }

					<Link to={`/page/sponsors/`} className="Link Secondary-Link"><span>See all event sponsors</span></Link>
